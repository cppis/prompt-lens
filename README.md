...
PromptLens is an MCP (Model Context Protocol) server for Claude Desktop and Claude Code that makes your prompts visible — what the AI understood, what it guessed, and what it couldn't know.

Trigger with `>> anz`, `>> 분석`, or just say "analyze this prompt."
No API key required for local mode.

> Get PromptLens installed and analyze your first prompt in 5 minutes.
...
Run the setup script to register PromptLens with Claude Desktop automatically:

```bash
./scripts/setup-claude-desktop.sh
```

The script detects your OS and adds PromptLens to `claude_desktop_config.json` without affecting existing MCP configurations.
...
When registered successfully, the MCP tool icon (🔧) appears in the bottom-right of the input area and `promptlens` is listed in the tools menu.
...
If `promptlens` appears in the output, you're good to go.

---

## Step 3. Analyze Your First Prompt

PromptLens is **triggered automatically** when it detects any of the following:
...
PromptLens responds automatically:
...
Type `/` in the Claude Desktop chat input to access PromptLens workflow presets:
...
| Create a project | `"Create a project called 'my-project' in PromptLens"` |
